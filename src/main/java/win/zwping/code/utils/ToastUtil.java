...
public class ToastUtil {
    ...
    private static void show(final int resId, final int duration) {
        try {
            CharSequence text = Util.getApp().getResources().getText(resId);
            show(text, duration);
    ...
    private static void show(final CharSequence text, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                iToast = ToastFactory.makeToast(Util.getApp(), text, duration);
    ...
    private static void show(final View view, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                iToast = ToastFactory.newToast(Util.getApp());
                iToast.setView(view);
    ...
    private static View getView(@LayoutRes final int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) Util.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    static class ToastWithoutNotification extends AbsToast {
        ...
        private static final Util.OnActivityDestroyedListener LISTENER =
                new Util.OnActivityDestroyedListener() {
                    @Override
        ...
        public void show() {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N_MR1) {
                ...
                Context topActivityOrApp = Util.getTopActivity();
                if (!(topActivityOrApp instanceof Activity)) {
                    Log.e("ToastUtil", "Couldn't get top Activity.");
                    return;
                }
                Activity topActivity = (Activity) topActivityOrApp;
                if (topActivity.isFinishing() || topActivity.isDestroyed()) {
                    Log.e("ToastUtil", topActivity + " is useless");
                    return;
                }
                mWM = topActivity.getWindowManager();
                mParams.type = WindowManager.LayoutParams.LAST_APPLICATION_WINDOW;
                Util.getActivityLifecycle().addOnActivityDestroyedListener(topActivity, LISTENER);
            } else {
                mWM = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            ...
            mParams.packageName = Util.getApp().getPackageName();
